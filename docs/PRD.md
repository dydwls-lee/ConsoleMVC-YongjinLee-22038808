...
1. 메인 메뉴 표시: 시료 관리 / 시료 주문 / 주문 승인·거절 / 모니터링 / 생산라인 조회 / 출고 처리 / 종료 (6개 + 종료)
...
| 시료 주문 | 시료 ID·고객명·수량 입력 검증, `RESERVED` 주문 생성 요청. 추가로 전체 주문 조회(상태 무관, `REJECTED` 포함, 읽기 전용) 제공 — 결정 근거: 루트 [docs/DECISIONS.md](../../../docs/DECISIONS.md) |
...
- 시료 주문 메뉴의 "전체 주문 조회"에서 `REJECTED` 상태 주문도 조회된다 (모니터링 메뉴와 달리 제외하지 않음).
